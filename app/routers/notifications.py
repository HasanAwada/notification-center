...
async def get_notifications(
    ...
    if search_text:
        query = query.filter(
            Notification.title.ilike(f"%{search_text}%")
            | Notification.content.ilike(f"%{search_text}%")
        )
    ...
    serialized_notifications = [
        NotificationOut.from_orm(notification) for notification in notifications
    ]

    response = {"count": total_count, "data": serialized_notifications}
...
async def mark_notifications_read(
    ...
    query = select(Notification).where(
        Notification.user_id == request.user_id, Notification.deleted_at.is_(None)
    )
    ...
    return {
        "detail": f"{len(notifications)} notifications marked "
        f"as {'read' if request.read else 'unread'}"
    }
...
async def delete_notification(
    ...
    if not db_notification:
        raise HTTPException(
            status_code=404, detail="Notification not found or unauthorized access"
        )
